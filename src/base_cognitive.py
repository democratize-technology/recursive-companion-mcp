...
from convergence import ConvergenceDetector, ConvergenceConfig, create_detector_for_tool
...
class CognitiveEnhancer:
    ...
    def _setup_convergence(self):
        ...
        if self.config.enable_convergence:
            ...
            logger.info(f"Convergence detection enabled with threshold {self.convergence_detector.config.threshold}")
    ...
    def add_iteration(self, content: str, metadata: Optional[Dict[str, Any]] = None) -> IterationResult:
        ...
        for iteration in range(1, max_iter + 1):
            ...
            if len(self.iteration_history) >= 2:
                ...
                if converged:
                    ...
                    logger.info(f"Iteration {iteration} similarity: {score:.4f} (threshold: {self.convergence_detector.config.threshold:.4f})")
    ...
    def _create_final_result(self) -> Dict[str, Any]:
        ...
        convergence_scores = [r.convergence_score for r in self.iteration_history if r.convergence_score is not None]
        
        return {
            "success": True,
            "final_result": final_iteration.content,
            "total_iterations": len(self.iteration_history),
            "convergence_achieved": final_iteration.convergence_score >= self.convergence_detector.config.threshold if self.convergence_detector else False,
            ...
            "metadata": {
                ...
                "threshold": self.convergence_detector.config.threshold if self.convergence_detector else None
            }
        }
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.iteration_history:
            scores = [r.convergence_score for r in self.iteration_history if r.convergence_score is not None]
            if scores:
...
class EnhancedThinkingTool(ABC):
    ...
    async def process(self, initial_input: str) -> Dict[str, Any]:
        ...
        return await self.enhancer.process_with_convergence(
...
def with_convergence(tool_name: str, threshold: Optional[float] = None):
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            return {
                "result": result,
                "enhanced_stats": enhancer.get_stats()
            }
